...
class Text extends Tool {
    ...
    didBecomeActive(lc) {
        ...
        this.unsubscribe = () => {
            return Array.from(unsubscribeFuncs).map(func => func());
        };
    ...
    _setShapesInProgress(lc) {
        switch (this.currentShapeState) {
            ...
            case "editing":
                return lc.setShapesInProgress([
                    this._getSelectionShape(lc.ctx, "rgba(0, 0, 0, 0.3)"),
                ]);
            default:
    ...
    _updateInputEl(lc, withMargin) {
        ...
        this.inputEl.style.left = `${lc.position.x / lc.backingScale +
            br.x * lc.scale -
            4}px`;
        this.inputEl.style.top = `${lc.position.y / lc.backingScale +
            br.y * lc.scale -
            4}px`;

        if (withMargin && !this.currentShape.forcedWidth) {
            this.inputEl.style.width = `${br.width +
                10 +
                this.currentShape.renderer.emDashWidth}px`;
        } else {
            this.inputEl.style.width = `${br.width + 12}px`;
        }

        if (withMargin) {
            this.inputEl.style.height = `${br.height +
                10 +
                this.currentShape.renderer.metrics.leading}px`;
        } else {
